...
def main():
	...
	prev_IP = None # Previous IP Address (In case IP changes)
	...
	while True:	
		...
		# if IP Address = present & different from before
		if myIPAddress and myIPAddress != prev_IP:
			LCD_String("My IP Address:", LCD_LINE_1)
			LCD_String(f"{myIPAddress}", LCD_LINE_2)
			prev_IP = myIPAddress # Set new IP Address as Previous
		
		# If IP Address not present
		elif not myIPAddress:
			LCD_String("Waiting for IP", LCD_LINE_1)
			LCD_String("Assignment...", LCD_LINE_2)
